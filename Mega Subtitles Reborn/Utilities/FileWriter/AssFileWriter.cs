...
namespace Mega_Subtitles_Reborn.Utilitis.FileWriter
{
    public class AssFileWriter
    {
        ...
        public static void WriteAssFile(List<string?> selectedActors, string assFilePath, bool isSingleFile = false, bool zeroLine = false, bool addTenSec = false, int template = 1)
        {
            try
            {
                ...
                if (isSingleFile)
                {
                    ...
                    Succses(assFilePath, true, template);
                ...
                else
                {
                    ...
                    Succses(assFilePath, false, template);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void Succses(string OutputPath, bool isSingleFile, int template)
        {
            try
            {   if (template != 1)
                    return; // If template is not 1, do not show success message
