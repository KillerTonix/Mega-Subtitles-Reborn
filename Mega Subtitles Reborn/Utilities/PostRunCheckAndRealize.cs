    ...
    public class PostRunCheckAndRealize
    {
        ...
        public static void CheckAndRealize()
        {
            try
            {
                if (File.Exists(GeneralSettings.Default.ProjectCahceJsonPath))
                {
                    ...
                    if (!File.Exists(subtitlesData.SubtitlesPath))
                    {
                        if (GeneralSettings.Default.Language == "English")
                            MessageBox.Show("Subtitles file not found. Please select a subtitles file.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        else
                ...
                {
                    if (GeneralSettings.Default.Language == "English")
                        MessageBox.Show("Project cache file not found. Please select a subtitles file.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    else
            ...
            {
                ExceptionLogger.LogException(ex, "PostRunCheckAndRealize", MethodBase.GetCurrentMethod()?.Name); // Log any exceptions 
            }
        }
    }
}
